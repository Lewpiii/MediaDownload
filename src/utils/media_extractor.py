...
class MediaExtractor:
    ...
    @classmethod
    def extract(cls, msg: discord.Message, allowed_exts: List[str], include_text_links: bool = False) -> List[Tuple[str, str]]:
        ...
        relaxed = os.getenv('RELAXED_ATTACHMENT_FILTER', 'false').lower() in ('1', 'true', 'yes')
        ...
        if include_text_links:
            # Note: This will likely fail if message content is not available, but caller should know that.
            # We catch AttributeError just in case.
            try:
                content = getattr(msg, 'content', None)
                if content:
                    for link in cls._extract_links_from_text(content):
